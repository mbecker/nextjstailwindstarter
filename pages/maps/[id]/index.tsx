  ...
  return (
    ...
    >
        ...
        {activity !== null && (
          <div className="sm:flex-auto">
            ...
            <div className="mt-4 text-sm text-slate-600">
              ...
              <span className="ml-2">{normalizeMovingTime(activity.moving_time)}</span>
              <span className="ml-2">{normalizeDistance(activity.distance)}</span>
            </div>
          </div>
        )}
      </div>
      <div className="mt-8 flex items-center justify-center">
        {
          activity !== null && (
            <Image src={`${process.env.NEXT_PUBLIC_POCKETBASE_URL}/api/strava/map/${activity.id}?width=800&height=600`} width={800} height={600} alt={activity.name}/>
          )
        }
      </div>
    </div>
